...
void writeCompressedOutput(const char * inFile,
  ...
  char c;
  while (fileStream.get(c).good()) {
    unsigned char uc = c;
    std::map<unsigned, BitString>::const_iterator cit = theMap.find(uc);
    assert(cit != theMap.end());
    bfw.writeBitString((*cit).second);
  }
  //dont forget to lookup 256 for the EOF marker, and write it out.
  std::map<unsigned, BitString>::const_iterator cit = theMap.find(256);
  assert(cit != theMap.end());
  bfw.writeBitString((*cit).second);
